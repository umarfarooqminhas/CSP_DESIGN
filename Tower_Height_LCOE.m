...
%% %% Dispatch Strategy
data = readtable('Tower Height Field Variation.csv');

Xaxis=data{1,3:15};
Yaxis=data{17,3:15};
for t=1:13

Tower_Height=190+t*5;
h=14;
 Hours_storage=h;
...
AEP= sum(W_NET_Tab)*Yaxis(t)/Yaxis(13);
...
%% Hours Storage Sensitivity Check
    ...
    while err_tank_D>10^-4
     ...
     D_tank= D_tank_guess;                           
    [LCOE] = LCOE_Cacls(A_field,Tower_Height,Reciever_Dia,Receiver_Height,PB_nom,D_tank,H_Tank,Salt_Inventory,AEP);
    LCOE_t(t)=LCOE;
end
...
hold on;
yyaxis right;
plot(Xaxis,LCOE_t,'o-',LineWidth=1.5)  ; 
ylim([115 130])
ylabel('LCOE [$/MWH') 
title('Tower Height Optimization');
xlabel('Tower Height [m]') ;

legend({'ETA OPTICAL', 'LCOE [$/MWH'})